...
SELECT COUNT(videogame_id) AS numero_recensioni, videogame_id
FROM reviews
GROUP BY videogame_id;
...
SELECT COUNT(name), YEAR(release_date)
FROM videogames
...
SELECT DISTINCT videogames.name, tournaments.year, tournaments.name
...
SELECT software_houses.name AS software_house, awards.name AS awards_name, videogames.name AS videogame_name
...
SELECT DISTINCT categories.name AS categoty_name, pegi_labels.name AS pegi, videogames.name
FROM videogames
...
INNER JOIN tournament_videogame ON videogames.id = tournament_videogame.videogame_id
INNER JOIN tournaments ON tournament_videogame.tournament_id = tournaments.id
WHERE award_videogame.award_id = 1 AND award_videogame.year = 2018;
...
INNER JOIN players ON player_tournament.player_id = players.id
WHERE award_videogame.award_id = 5 AND award_videogame.year = 2018 AND tournaments.year = 2019;

--*********** BONUS ***********

--10- Selezionare i dati della prima software house che ha rilasciato un gioco, assieme ai dati del gioco stesso (software house id : 5)
SELECT TOP 1 software_houses.name, videogames.*
FROM videogames
INNER JOIN software_houses ON videogames.software_house_id = software_houses.id
...
SELECT TOP 1 videogames.id, videogames.name, videogames.release_date, COUNT(reviews.videogame_id) AS totale_recensioni
FROM reviews
INNER JOIN videogames ON reviews.videogame_id = videogames.id
GROUP BY videogames.id, videogames.name, videogames.release_date ORDER BY totale_recensioni DESC;

--12- Selezionare la software house che ha vinto più premi tra il 2015 e il 2016 (software house id : 1)
SELECT TOP 1 software_houses.id, COUNT(videogames.software_house_id) AS winner
FROM award_videogame
INNER JOIN videogames ON award_videogame.videogame_id = videogames.id
INNER JOIN software_houses ON videogames.software_house_id = software_houses.id
WHERE award_videogame.year BETWEEN 2015 AND 2016
GROUP BY software_houses.id ORDER BY winner DESC;

--13- Selezionare le categorie dei videogame i quali hanno una media recensioni inferiore a 1.5 (10)
SELECT DISTINCT videogames.name, categories.name
FROM videogames
INNER JOIN reviews ON videogames.id = reviews.videogame_id
INNER JOIN category_videogame ON videogames.id = category_videogame.videogame_id
INNER JOIN categories ON category_videogame.category_id = categories.id
GROUP BY categories.name, videogames.name
